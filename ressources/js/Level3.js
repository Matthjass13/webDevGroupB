...
export class Level3 {
  constructor(ctx, game) {
    ...
    this.bgImage.src = "ressources/images/game/level/background/level3.png";
    ...
    this.NB_COINS = 20; // Number of coins to spawn in the level
    ...
    this.key = new Key(500, 280); // Position of the key
  ...
  update(modifier) {
    ...
    if (!this.key.collected && this.pirate.touchElement(this.key)) {
      ...
      this.walls = this.walls.filter(
          (wall) => !(wall.x === 195 && wall.y === 85) // Remove the door from the walls array
      );
    }
